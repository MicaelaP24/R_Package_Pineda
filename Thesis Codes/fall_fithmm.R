...
noctules_fall <- read.csv("/cloud/project/Thesis data/Fall nights/Fall.csv")
library(tidyverse)
library(lubridate)
noctules_fall$timestamp <- ymd_hms(noctules_fall$timestamp)
noctules_m <- move(x = noctules_fall$location.long,
                   y = noctules_fall$location.lat,
                   time = noctules_fall$timestamp,
                   data = noctules_fall,
                   proj = CRS("+proj=longlat +ellps=WGS84"),
                   animal = noctules_fall$individual.local.identifier)
#str(noctules_m)
dev.off
dev.new()
plot(noctules_m)
...
X6E6B_3 <- as.data.frame(nocday_L$X6E6B.3)
write.csv(X6E6B_3, file = "/cloud/project/Thesis data/Fall nights/5 min/X6E6B_3.csv")
