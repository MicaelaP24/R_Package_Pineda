...
phen <- read_csv("/cloud/project/data/Phenology_data.csv")
...
unique(butterfly$ButterflySpecies)
...
ggplot(data = phen)+
  geom_line(aes(x = Year, y = AnnualTemp)) 
...
model_fit <- lm(AnnualTemp ~ Year, data = phen)
summary(model_fit)

ggplot(phen, aes(x = Year, y = AnnualTemp)) + 
  geom_point(size = 0.5) +        
  ...
  labs(x = "Year", 
       ...
       title = "Linear regression to predict Annual Temperature")
...
model_fit1 <- lm(Day ~ SpringTemp, data = butterfly)


ggplot(butterfly, aes(x = SpringTemp, y = Day)) + 
  geom_point(size = 0.5) +        
  stat_smooth(method = "lm",     
              color = "navy",    
              size = 1,        
              fill = "deeppink4") + 
  labs(x = "Spring Temperature", 
       y = " Day", 
       title = "Linear regression to predict Day of Emergence")


model_fit2 <- lm(Day ~ SummerTemp, data = butterfly)


ggplot(butterfly, aes(x = SummerTemp, y = Day)) + 
  geom_point(size = 0.5) +        
  stat_smooth(method = "lm",     
              color = "navy",    
              size = 1,        
              fill = "deeppink4") + 
  labs(x = "Summer Tempurature", 
       y = " Day of Emergence", 
       title = "Linear regression to predict Day of Emergence")



model_fit3 <- lm(Day ~ Year, data = butterfly)


ggplot(butterfly, aes(x = Year, y = Day)) + 
  geom_point(size = 0.5) +        
  stat_smooth(method = "lm",     
              color = "navy",    
              size = 1,        
...
```
#Answer here
No, they all have low r-squared values, with the hightest being in Spring temp/day of emergence. 17.75%. They also all have high Residual standard error with the lowest also in Spring temp/day of emergence 25.27. I would conclude from this no relationships are significant but Spring temp/day of emergence has the strongest correlation.
...
```{r}
#Answer here.
From the facet wrap you can visually observe there is a negative relationship between annual temperature and day of emergence for species. It varies between the species as mentioned in Kharouba and Vellend's paper, with some being more obvious than others. As adult flight and flowering occur earlier wih increasing temperature in the paper so does day of emergence with the data provided here.
